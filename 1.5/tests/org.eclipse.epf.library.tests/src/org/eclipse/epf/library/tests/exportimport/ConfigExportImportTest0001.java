 ...
 * 
 */
public class ConfigExportImportTest0001 extends ExportImportTestMethodBase {
	
	public ConfigExportImportTest0001(ExportImportTestImpl owner) {
		super(owner);
	}
	...
	public boolean execute(String testName) {
		...
		tcs[2] = service.newTestCommand(TCExportConfiguration.class);
		tcs[2].setAttribute(TestCommand.AT_ExportFolderName, "Export0001");
		tcs[2].setAttribute(TestCommand.AT_SelectedConfigName, "Config A");

		tcs[3] = service.newTestCommand(TCOpenLibrary.class);
		tcs[3].setAttribute(TestCommand.AT_LibFolderName, "Base_1");
		
		tcs[4] = service.newTestCommand(TCImportConfiguration.class);
